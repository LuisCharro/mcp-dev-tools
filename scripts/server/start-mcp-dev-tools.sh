...
if [ -z "${REPO_ROOT:-}" ]; then
    ...
    echo -e "${GREEN}Saved REPO_ROOT to $PROJECT_ROOT/.env.local${NC}"
fi
...
export REPO_ROOT
"$THIS_DIR/start-http.sh" --port $PORT &> "$PROJECT_ROOT/mcp-gateway.log" &
SERVER_PID=$!
...
if ps -p $SERVER_PID > /dev/null; then
    ...
    echo "📊 Server Info:"
    echo "  - PID: $SERVER_PID"
    echo "  - Logs: tail -f $PROJECT_ROOT/mcp-gateway.log"
    echo ""
    ...
    echo "Check logs: cat $PROJECT_ROOT/mcp-gateway.log"
    exit 1
fi
